...
class Shapely2ESRI():
    ...
    def addField(self, name, fieldType='STR', length=None, decimals=None, default=None, pyType=str):
        ...
        if length is None and decimals is None and default is None:
            field = klass(name)
        elif length is None and decimals is None:
            field = klass(name, default=default)
        elif length is None and default is None:
            field = klass(name, decimals=decimals)
        elif decimals is None and default is None:
            field = klass(name, length=length)
        elif length is None:
            field = klass(name, decimals=decimals, default=default)
        elif decimals is None:
            field = klass(name, length=length, default=default)
        elif default is None:
            field = klass(name, length=length, decimals=decimals)
        else:
            field = klass(name, length=length, decimals=decimals, default=default)
